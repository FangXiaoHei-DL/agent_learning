...
**发表**：2026 年 3 月 13 日 | [arXiv:2603.09619](https://arxiv.org/abs/2603.09619)

**核心贡献**：本文将"上下文工程"正式定义为独立学科，提出五大上下文质量标准（相关性、充分性、隔离性、经济性、溯源性），并构建四层学科金字塔：提示词工程 → 上下文工程 → 意图工程 → 规范工程。研究发现 75% 的企业计划在两年内部署 Agent AI，但普遍存在"上下文赤字"和"意图赤字"——模型得到了信息但缺乏决策所需的组织目标与约束语境。

**与本章关系**：与本章 7.1 节"上下文工程 vs 提示工程"的讨论直接呼应，为本章整体框架提供了最新的理论支撑与企业实践视角。

---

### [CacheFlow：3D 并行 KV Cache 恢复加速长上下文 LLM 服务](https://arxiv.org/abs/2604.25080)

**发表**：2026 年 4 月 28 日 | [arXiv:2604.25080](https://arxiv.org/abs/2604.25080)

**核心贡献**：KV Cache 恢复已成为长上下文 LLM 服务（多轮对话、RAG、Agentic pipeline）的主要延迟瓶颈。本文提出 CacheFlow，将缓存恢复重新定义为跨 token、跨层、跨 GPU 的三维并行执行问题，引入批感知双指针调度器联合优化计算与 I/O 分配。在多个模型和硬件配置上将首 token 时间（TTFT）降低 **10%~62%**，显著改善 Agentic pipeline 的实时响应性。
