...
> 🗓️ 本节由每日自动更新任务维护，最近更新：**2026 年 5 月 7 日**
...
### [UniToolCall：统一工具调用表示、数据与评测的 LLM Agent 框架](https://arxiv.org/abs/2604.11557)

**发表**：2026 年 4 月 13 日 | [arXiv:2604.11557](https://arxiv.org/abs/2604.11557)

**核心贡献**：本文提出 UniToolCall，一个涵盖工具集构建、数据生成到评测的全链路标准化框架：整合 22k+ 工具、390k+ 条训练数据，引入 Anchor Linkage 机制强制跨轮依赖，将 7 个公开基准统一转化为 QAOA（Query-Action-Observation-Answer）格式，并在函数调用、轮次、对话三个粒度做细粒度评测。在工具干扰最强的 Hybrid-20 设置下，微调后的 Qwen3-8B 达到 93.0% 单轮严格精度，超越 GPT、Gemini 和 Claude 等商业模型。

**与本章关系**：直接对应本章工具调用的数据与评测痛点，为本章 6.3 节的 Benchmark 比较提供了最新统一评测基准。

---

### [Reinforced Agent：推理时工具调用反馈的主动纠错机制](https://arxiv.org/abs/2604.27233)

**发表**：2026 年 4 月 29 日 | [arXiv:2604.27233](https://arxiv.org/abs/2604.27233)

**核心贡献**：传统工具调用评估只能事后发现错误，本文提出将评估前移到推理时执行循环中：引入专门的 Reviewer Agent 在工具执行前审查临时工具调用，将范式从"事后恢复"转变为"主动预防"。实验表明，该方法在 BFCL 单轮场景提升 +5.5%、在 Tau2-Bench 多轮有状态场景提升 +7.1%，结合 GEPA 自动提示优化可进一步获得 +1.5~2.8% 的改进。

**与本章关系**：对应本章工具调用可靠性与错误恢复的知识点，是对现有 ReAct 式工具调用流程的在线增强方案。

---

### [调还是不调：评估与优化 LLM 工具调用决策的统一框架](https://arxiv.org/abs/2605.00737)

**发表**：2026 年 5 月 1 日 | [arXiv:2605.00737](https://arxiv.org/abs/2605.00737)

**核心贡献**：工具调用并非总是有益——冗余或有害的调用会降低 Agent 性能。本文提出一个评估工具调用决策的三维框架：**必要性**（是否真的需要工具）、**效用**（工具是否提供价值）、**可负担性**（调用成本是否合理）。研究发现模型对工具调用的"主观认知"与"客观效果"之间存在系统性错位，并训练基于隐藏状态的轻量级估计器来纠正决策偏差，在 3 个任务、6 个模型上均优于原有自感知设置。
