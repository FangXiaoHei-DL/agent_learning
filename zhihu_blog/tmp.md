...
![零基础读者进入大模型世界的路线图](../src/zh/svg/chapter_llm_00_beginner_map.svg)
...
![大模型不是搜索引擎，而是语言协作者](../src/zh/svg/chapter_llm_00_llm_vs_search.svg)
...
![把大模型当成能力很强但需要检查的实习生](../src/zh/svg/chapter_llm_00_ai_intern_loop.svg)
...
![大模型能帮普通人做什么](../src/zh/svg/chapter_llm_00_capability_map.svg)
...
![新手最实用的提问公式](../src/zh/svg/chapter_llm_00_prompt_formula.svg)
...
![token 是模型眼里的文字小颗粒](../src/zh/svg/chapter_llm_00_token_particles.svg)
...
![一次调用的 token 成本从哪里来](../src/zh/svg/chapter_llm_00_token_cost.svg)
...
![不同任务的 token 消耗阶梯](../src/zh/svg/chapter_llm_00_cost_pyramid.svg)
...
很多新手最关心的问题是：**我到底该用哪个模型？**

这个问题没有永远正确的答案，因为模型更新很快，价格也经常变化。更好的方式不是背排行榜，而是按任务选择。

![怎么选模型：先看任务，不看排行榜](../src/zh/svg/chapter_llm_00_model_choice_tree.svg)

你可以先用下面这张表建立一个大致印象：

| 模型 / 产品 | 适合场景 | 简单评价 |
|---|---|---|
| **ChatGPT / GPT 系列** | 通用问答、写作、代码、工具生态 | 综合能力强，生态成熟 |
| **Claude 系列** | 长文档阅读、写作、代码理解、严谨表达 | 长文本和文字质量通常很强 |
| **Gemini 系列** | 多模态、长上下文、Google 生态 | 适合图像、视频、长资料场景 |
| **DeepSeek 系列** | 中文、代码、推理、性价比 | 成本优势明显，开发者常用 |
| **Qwen / 通义千问** | 中文、本地部署、企业应用 | 国内生态好，开源模型丰富 |
| **Kimi** | 中文长文档、资料阅读 | 面向普通用户的长文本体验好 |
| **豆包 / 文心 / GLM 等** | 中文办公、内容生成、国内应用集成 | 适合国内产品环境和日常使用 |
| **Llama / Mistral 等开源模型** | 本地部署、私有化、研究 | 适合有技术能力的团队 |

如果你不想纠结，可以这样选：

- **日常聊天和办公**：选一个你觉得稳定、便宜、顺手的。
- **写代码**：优先试 GPT、Claude、DeepSeek、Qwen。
- **读长文档**：优先试 Claude、Gemini、Kimi。
- **中文内容**：DeepSeek、Qwen、Kimi、豆包、文心、GLM 都值得试。
- **需要看图、读截图、分析图表**：选支持多模态的模型。
- **需要低成本或私有化**：关注 DeepSeek、Qwen、Llama、Mistral 等模型或开源方案。
- **需要最强综合能力**：选 GPT、Claude、Gemini 等高端版本做对比测试。

最重要的是：

> **不要问“哪个模型永远最好”，而要问“我的任务适合哪个模型”。**

对于普通用户来说，最好的入门方式不是一次性研究所有模型，而是固定使用一两个主力模型，然后在遇到特殊任务时再换模型测试。
...
![从聊天模型到 AI 工具：应该选哪个](../src/zh/svg/chapter_llm_00_agent_tool_choice.svg)
...
![大模型最容易犯的五类错](../src/zh/svg/chapter_llm_00_risk_cards.svg)
...
![正确使用大模型的闭环](../src/zh/svg/chapter_llm_00_use_loop.svg)

从这里往后，你可以继续学习几个更进阶的主题：

| 下一步主题 | 你会学到什么 |
|---|---|
| **Prompt Engineering** | 怎样设计更稳定、更可控的问题 |
| **模型 API** | 怎样在自己的程序里调用大模型 |
| **上下文窗口** | 为什么长对话和长文档会影响效果与成本 |
| **RAG** | 怎样让模型基于你自己的资料回答问题 |
| **Agent** | 怎样让模型不只回答，还能规划、调用工具、执行任务 |
| **模型评测** | 怎样判断一个模型或提示词是不是真的更好 |

> **把任务说清楚，把结果当成初稿，把重要内容再验证。**
